...
class App:
    def __init__(self, root):
        ...
        # DF1
        tk.Label(left, text="Client給的檔案：").grid(row=0, column=0, sticky="w")
        tk.Label(left, textvariable=self.df1_path, width=50, anchor="w").grid(row=0, column=1)
        tk.Button(left, text="選擇 Client File", command=self.load_df1).grid(row=0, column=2, padx=5)

        # DF2
        tk.Label(left, text="EU OO檔案：").grid(row=1, column=0, sticky="w")
        tk.Label(left, textvariable=self.df2_path, width=50, anchor="w").grid(row=1, column=1)
        tk.Button(left, text="選擇 EU OO File", command=self.load_df2).grid(row=1, column=2, padx=5)

        # 處理按鈕
        tk.Button(left, text="算出上個星期二 Client->EU OO", width=20, command=self.process_files).grid(row=2, column=1, pady=20)

        # ====== 右側 Frame (輸出訊息區) ======
        right = tk.Frame(root)
        right.pack(side="right", padx=10, pady=10)

        tk.Label(right, text="輸出訊息 Client->EU OO 上個星期二").pack()
        self.output = tk.Text(right, width=80, height=60)
        self.output.pack()
    ...
    def load_df1(self):
        ...
        if path:
            self.df1_path.set(path)

    # --- DF2 選擇器 ---
    def load_df2(self):
        path = filedialog.askopenfilename(filetypes=[("Excel Files", "*.xlsx")])
        if path:
            self.df2_path.set(path)

    # --- 處理主程式（你要把 df1/df2 → df_new 的程式放這裡） ---
    def process_files(self):
        self.output.delete("1.0", tk.END)

        if not self.df1_path.get() or not self.df2_path.get():
            self.output.insert(tk.END, "請先選擇檔案們\n")
            return
        ...
        try:
            ...
            # ====== 顯示在右邊訊息區 ======
            self.output.insert(tk.END, " Number of Rows = {} \n".format(len(df_new)))
            self.output.insert(tk.END, df_new.to_string(index=False))

            df_new.to_csv("df_new.csv", index=False, encoding="utf-8-sig")

        except Exception as e:
            self.output.insert(tk.END, f"發生錯誤：{e}\n")
